...
export default function Inlet(props) {
  ...
  const onFinish = (values) => {
    ...
    fetch(
      `/api/inlets/create?email=${session.user.email}&name=${name}&description=${desc}`,
      {
        method: "POST",
      }
    ).then((res) => {
  ...
  return (
    <>
          ...
          <Collapse bordered={false} defaultActiveKey={["0"]} ghost>
            <Panel
              header={
                ...
                >
                  {inlet.id == 0
                    ? "Select an inlet to view the code"
                    : inlet.name}
                </Space>
              }
            >
      ...
      <Row>
        <Col span={24}>
          <Editor inlet={inlet} fetchinlet={fetchinlet} onCodeRan={props.onCodeRan}></Editor>
        </Col>
      </Row>
    </>
  );
