...
function Editor(props) {
  ...
  const onSave = (values) => new Promise((resolve, reject) => {
  ...
  return (
    <div style={{paddingBottom: 35}}>
      <Space style={{ marginBottom: 16, marginTop: 6 }}>
        <Button
          ...
          onClick={() => {
            ...
            let maybeSave = (props.inlet && props.inlet.code !== code) ? onSave() : Promise.resolve();

            maybeSave.then(() => fetch(
      ...
      <CodeEditor
        value={data}
        language="js"
        placeholder="//you can attach a data string"
        onChange={(evn) => {
      ...
      <CodeEditor
        value={codeLog}
        disabled={true}
        placeholder="ithemLog() statements and errors show up here"
        onChange={(evn) => {}}
        minHeight={20}
        padding={15}
        style={{
    ...
    >
      <Table
        ...
        columns={[
          {
            ...
            key: "function",
            width: 300,
            render: (text) => <Typography.Text code={true} copyable={true}>{text}</Typography.Text>
          },
          {
            title: "description",
        ...
        dataSource={[
          ...
          {
            function: `ithemSchedule("outletName", time, data)`,
